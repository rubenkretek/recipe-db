...
- **RLS validates the `kitchen_id` column, not the relationship.** `is_kitchen_member(kitchen_id)` proves the caller is in the kitchen the row *claims* — it does not check that the rows being joined live there too. A user in two kitchens can write a join row that straddles them, and every member of the stated kitchen can then read it. Fixed on `ingredient_supermarkets` in Phase 5 with **composite foreign keys**: the parent gets a unique constraint on `(id, kitchen_id)` and the join table references that pair, so a mismatch cannot be written at all. **`recipe_tags`, `recipe_ingredients`, `recipe_photos` and `ratings` still have this gap** — verified open, not theoretical. Any new join table must use the composite shape.
