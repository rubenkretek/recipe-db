...
- **Shopping ticks are optimistic but not live.** `useOptimistic` makes a tap land instantly on a patchy supermarket connection, but two phones do **not** see each other until a refresh. Realtime, the Dexie queue and TanStack Query are all Phase 8 — do not add TanStack early, there is no cache for it to serve yet.
