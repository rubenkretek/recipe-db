...
- **The method is `recipe_steps`, not `recipes.method`.** Since 2026-09-12 (SPEC.md §9 decision 4, reversed). `recipes.method` is kept only as the undo for the migration that copied it into steps — nothing reads it and nothing may write to it; a later migration drops it. Anything new that produces a method, including the Phase 10 importer, must produce steps.
